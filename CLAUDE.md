...
## 調査済み: 再生バー・タイムラインの残像 (2026-09-25: アプリ仕様と判断・Wine 修正なし)

### 確定した機構 (D2D トレースで証明)
- メイン UI に Win32 子窓なし。UI 全体は単一 D2D hwnd ターゲット (`Resize 1920x1129`) に直接描画
- 再生中の描画は5フレーム周期のラウンドロビン (プレビュー→バー→区切り→…)＋プレイヘッドの増分 `DrawLine`
- プレイヘッドは**消去なし・同ブラシ・x 単調増加**で描かれる (6秒間に tall vertical 28本を確認)。
  アプリ自身のコマンド列に消去呼び出しが存在しないため、Wine が忠実に再現した結果が残像になる
- 全体再描画 (再生開始・停止・スクロール・ループ巻き戻し) で残像は消える。停止直後の撮影では残像なしを確認
- 背景が崩れない＝サーフェス保持が成立している (保持がなければ UI 全体がガベージになる)。
  保持＋消去なし増分描画の組み合わせは Windows でも同じ絵になるため、アプリ側の描画方式の問題と判断

### 対応
- Wine 側の修正はしない (存在しない描画を捏造も、UI 全体が依存する保持の破棄もできない)
- 回避策: 停止→再生・スクロール等で全体再描画を促す
- 調査用に `x86_64-w64-mingw32-gcc` で Win32 子窓列挙ツールを作成可 (`/tmp/opencode/inspect.c` 参照。
  `/tmp` は揮発のため必要なら再作成すること)

#### 追試 (2026-09-26: 保持ハック必須を確認・revert 済み)
ユーザーの「Windows では正常」との指摘を受け、以下を実験:
- `presentOptions=0x2` (DISCARD・保持要求なし) を確認。アプリは保持を要求しないのに増分描画する
- Sleep 16ms (vsync リズム模倣): 残像に変化なし → pacing/ポンプリズム無関係
- 保持ハック除去 (DISCARD→UNDEFINED 復帰): UI 全体が崩壊 (May の黒画面の再現) → **保持は必須**。
  即 revert し現行構成 (Sleep2＋保持) に復帰・デプロイ・UI 復帰確認済み
- 結論維持: アプリの消去なし増分描画＋保持前提が原因。Windows との差の残候補は
  タイムラインのズーム状態 (追従スクロールによる全体再描画の頻度差)。ユーザーに確認依頼中
...
コアを絞るとスケジューリングが安定し確実に揃う。**起動は `./aviutl2.sh` を使用すること**
(`TASKSET_CPUS` 環境変数でコア集合を変更可。`8-11` でも同等を確認済み (2026-09-21) のため
効くのはコア数 (4) であり配置ではない。6/8 コア等の中間値は未検証)。
...
- **新発見 (2026-09-21)**: Space は `F_big` (キー入力ハンドラ, arrow 0x25-28 判定あり) で分岐する。
  仮想呼出 `+0x18` が非ゼロを返すか、フィールド `+0xb0` が非 NULL の場合はトグル連鎖 (`0x140240f70`)
  に入らず別経路 (仮想 `+0x80` 等) へ  向かう。再生中の `+0xb0` 内容 (stale の可能性) が停止成否を
  分けている疑い。`+0xb0` 書き込みは初期化以外に複数あり型特定が残課題
...
### ✅ 解決: 停止遅延の決着 (2026-09-23: 0〜0.5秒・ave 0.2秒)
**決定打はアプリ設定だった**: 「設定→キャッシュサイズの設定→プレビュー再生の
バッファリング時間」を 500ms → 最小値 (100) に変更したところ、停止遅延が
0〜0.5秒 (ave 0.2秒) に短縮。デコード先行バッファの排出が遅延の正体だったことが確定。
- Wine 側の寄与 (有界スロットル・ポンプ除去・affinity) は基盤として維持。
  (FIFO pacing はカクつきのため revert。MAILBOX＋有界スロットルが現行構成)。
  単独では 0〜4秒止まりだったものが、バッファ最小化と合わさって実用域に到達した
- 最速の停止手順: Space 1回＋短時間待機。再押下はトグルで再生再開するため逆効果
- メモ: 診断ログ大量出力自体が遅延を悪化させるため、遅延測定時は診断を外すこと

#### 残存分散の最終評価 (2026-09-24: ポーリング churn として凍結)
軽動画再生中の CPU プロファイル (`tools/sample_cpu.sh`) で、アプリ3スレッドが
10秒間に 7.6s＋5.0s＋5.0s (計1.8コア相当) を消費し続けることを確認。
CS スレッドは駐留 (futex)・メインはほぼ idle のため、映像・デコード系が
イベント待ちではなくポーリングで回っている。停止フラグの観測はこのポーリングに
依存し、Wine 上のタイミングでは成功したり (即時〜数秒) 外れたり (〜29秒) する。
affinity 制約・pacing・バッファ最小化はいずれもこの churn を鎮める方向に効くが、
ポーリング自体 (アプリ設計) は変えられないため完全な即時性には至らない。
現状 (単発 Space＋待機で 0〜4秒安定・フリーズなし) で実用決着とする。

#### 除外済み: GOP/キーフレーム依存 (2026-09-25)
1秒 GOP と 10秒 GOP のテストクリップ (`~/Videos/gop_test/`) で停止遅延に差なし。
GOP 境界待ち説は棄却。

#### 追加実験の結果 (2026-09-24: いずれも効果なし・除去済み)
- micro-wait (50ms 毎の alertable 2ms 待機): 待機飢餓説を検証したが無効。待機は原因ではない
- 詳細は worktree のコミット履歴 (`EXPERIMENT`/`Revert` 系) を参照。除去後の構成は
  FIFO-revert 後の実証セット (MAILBOX＋有界スロットル＋ポンプ除去) に一致する

### 🧊 凍結: 残存分散の原因は GPU ストール (2026-09-24: 調査終了・文書化)
停止遅延の残存分散 (0〜29秒) を追跡し、映像系 presenting スレッド (`04fc`) が
`vkBeginCommandBuffer` 付近で最大 2.79 秒完全沈黙すること (`logs/stop_decomp.log` で確認)、
他スレッドは同時刻も正常動作することを確定。使用中コマンドバッファのフェンス待ち、
すなわち **GPU 側の数秒遅延**が原因とみられる。Mesa/RADV・電力・VRAM・コンポジタ絡みで
...
### ⚡ 進行中: FIFO pacing で空転除去＋電力回復 (2026-09-23〜)
taskset 実験 (`0-3` で 0.5〜1s 安定、`8-11` 同等、12 コアで 28s〜不能) と
クロック計測 (無制限 1.8〜2.8GHz / 1コア制約 3.4〜4.65GHz) から機構を確定:
毎秒数千の空転 present が電力を浪費→クロック低下→トグル逐次ハンドオフ遅延。
対処: `swap_interval==0` でも FIFO を最優先 (従来は MAILBOX/IMMEDIATE でノンブロック空転)。
FIFO は spec 必須のため実質つねに選択される。UI 重化・カクつき・フリーズが出たら revert。
検証待ち: 空転率・クロック回復・停止秒数・回帰の有無。

### ✅ FIFO 検証結果 (2026-09-23: 空転解消・クロック回復・停止3秒安定)
- acquire:present が 44:1 → **1.23:1** に回復 (空転除去)
- クロックが **4.7GHz** 台に回復 (制約なし・全コア使用のまま)
- 停止は **3s×3 で安定** (変動なし)。フリーズ・応答なしの報告なし
- 注意: present カウンタ上は映像スレッドが止まらず継続表示のため、
  「停止」は内容静止 (stale frame) とみられる。カウンタでは内容を見分けられない
  ため厳密な停止点の特定は不可。回帰 (UI重化・カクつき) が出たら報告すること

### ⚠️ カクつき報告を受け FIFO を revert 試験 (2026-09-24: 検証待ち)
停止反映改善後から再生プレビューにカクつき。affinity は A/B で除外済み。
次容疑の FIFO を revert (MAILBOX/IMMEDIATE に復帰) して切り分け中。
FIFO が原因なら、vsync ブロックが CS 直列化 (maps/destroy の停滞) や
16ms 量子化ジッタを起こしている可能性。結果次第で MAILBOX＋待機拡張等の
代替 pacing を検討。空転・電力・停止秒数の後退に注意。

### 🔄 Emit-counter pacing を実装 (2026-09-24: FIFO の代替・検証待ち)
FIFO がカクつきの原因と確定したため、ブロックしない pacing 方式を新規実装。
未実行 present op 数 (`outstanding_presents`) が `max_frame_latency` に達したら
emit 前に即時復帰 (ドロップ) する。待機なし (stall なし)・vsync 非依存
(量子化ジッタなし)・WAITABLE/モード非依存 (emit 層で動作)・デッドロックなし
(待機自体がない)。正常時は CS が捌き切りカウンタが溜まらず無害。
親パッチに反映・検証待ち: 空転率・停止秒数・カクつき解消の有無。

### ↩️ Emit-counter を revert (2026-09-24: 停止10秒化で無効確定)
検証結果: カクつきなし・停止 ave 10s (悪化)。S_OK 偽装のスキップがアプリの
停止ハンドシェイクを壊したとみられる (実行されない present を完了扱いにするため、
停止完了の検出・待機がずれる)。教訓: 未実行の present を成功扱いで捨ててはならない。
実証済み構成 (MAILBOX＋有界スロットル＋ポンプ除去) に復帰。

### 😴 Sleep pacing を実験 (2026-09-24: 検証待ち)
待機でもブロックでも嘘でもない第4の pacing: present 毎に 2ms スリープ。
排出上限 ~500/s (空転 4300/s の約1/9)、正常時オーバーヘッド約12% (60fps 換算で知覚不可)、
デッドロックなし (スリープは必ず復帰)、vsync 非依存、フラグ・モード非依存。
狙い: 電力浪費の削減 (クロック回復) と停止遅延の短縮。検証待ち:
空転率・クロック・停止秒数・カクつきの有無。

### 🔓 WAITABLE 除外を撤廃 (2026-09-24: 検証待ち)
有界スロットルが効かない映像経路の原因として、WAITABLE フラグ付きスワップチェーンの
待機スキップが濃厚 (init ログでは non-waitable のみ確認だが、flood 時の挙動と矛盾する
ため)。DXGI 的にはアプリ自己 pacing 用の除外だが、100ms 有界なら安全として全適用に変更。
健全時はセマフォ枯渇せず無害。検証待ち: 空転率・停止秒数・UI 応答性。

### ⚡ 16ms 化で停止短縮を狙う (2026-09-23: 理論棄却・revert 済み)
3 秒安定の内訳として「排出数十回 × 100ms タイムアウト」と推定し、16ms に短縮した。
結果: 停止は変わらず、代わりにバックアップ時の排出上限が 10/s → 60/s に上がり
空転が復活したため revert (100ms に復帰)。タイムアウト値では排出速度と空転率が
トレードオフし、両立しないことが確定。将来的な適応型バイパス案 (連続ドロップ時は
emit 自体をスキップ) は未実施。

### 🔄 適応型バイパスを実装 (2026-09-23: 効果なし・revert 済み)
トレードオフ回避を狙ったが、測定 (acquire:present 1.24:1 維持) で効果なしが確定。
FIFO pacing 下では持続ドロップ自体が起きず、バイパスが発火しないため。
複雑性のみ残るため除去し、実証済みの構成 (FIFO＋有界スロットル＋ポンプ除去) に戻した。
