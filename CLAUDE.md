...
最終状態で位置・再生時間によらず 0〜2 秒で停止することを確認。経緯と教訓:
- 有界スロットル (100ms 待機＋ドロップ不返却) で present 空転を pacing。無限待機版の黒画面ハングは
  起こさない。再生中の重さ増加なし、フリーズ・応答なしなし
- present 内ポンプは完全除去 (上流動作)。マウス中継のみ版でも停止遅延に差はなく、計測の
  クリーンさを優先した
- ⚠️ 測定方法論の教訓: 大量の `fprintf` 診断ログ自体が停止遅延を 1〜29 秒に悪化させていた
  (診断あり 1〜29秒変動 → 除去後 0〜2秒安定)。遅延測定時は診断を外すこと。ログは
  プロジェクト内 `logs/` に保存する (`/tmp` はクリーンで消える)
