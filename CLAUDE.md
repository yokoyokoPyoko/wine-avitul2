...
### ✅ 解決: 停止遅延の安定化 (2026-09-20: 0〜4秒で安定・実用決着)
最終測定 (単発 Space×3: 0s, 4s, 0.5s) で実用域に到達。フリーズ・応答なしなし。
完全な即時性には至らず。残る分散 (即時〜数秒) はアプリ内部のトグル状態機械に依存し、Wine 側からは
...
### 🔄 適応型バイパスを実装 (2026-09-23: 検証待ち)
トレードオフを回避する設計: 連続ドロップがしきい値 (5回) に達し、かつ burst 発行中
(UI レートの低頻度 present は対象外) の場合、wait＋emit をスキップして即時復帰し、
16回に1回だけ再試行する。成功で streak リセット。Interlocked 操作でスレッド安全。
期待: 停止排出・起動・リサイズ等のバックアップ局面で待機も発行もせず高速排出、
通常時は従来通り pacing。検証待ち: 空転率・停止秒数・回帰の有無。
