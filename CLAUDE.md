...
### ⚡ 16ms 化で停止短縮を狙う (2026-09-23〜・検証待ち)
3 秒安定の内訳を特定: 停止排出フェーズで acquire が系統的に NOT_READY
(ウィンドウ状態遷移・破棄中のため) になると、枯渇セマフォに対し毎回 100ms
タイムアウト待ちが発生し、排出数十回 × 100ms ＝ 約3秒の遅延になる。
対処: タイムアウトを 100ms → 16ms (1フレーム時間) に短縮。デッドロック安全性は
維持 (有限待機) し、バックアップ時の排出上限は約60/s (無制限の数千/s には程遠く
pacing として有効)。約0.5秒への短縮を見込む。変わらなければ理論棄却。
